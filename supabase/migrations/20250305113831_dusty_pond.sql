/*
  # Settings and Storage Configuration Fix
...
BEGIN
  -- Create logos bucket
  IF NOT EXISTS (SELECT FROM storage.buckets WHERE id = 'logos') THEN
  ...
  IF NOT EXISTS (SELECT FROM storage.buckets WHERE id = 'signatures') THEN
    INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
    VALUES (
      'signatures',
      'signatures',
  ...
  IF NOT EXISTS (SELECT FROM storage.buckets WHERE id = 'internal-rules') THEN
    INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
    VALUES (
      'internal-rules',
      'internal-rules',
