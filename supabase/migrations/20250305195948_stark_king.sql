/*
  # Create trainings table and add training fields to user profiles
  
  1. New Tables
    - `trainings`
      - Basic fields: id, title, description, duration, etc.
      - Training details: objectives, content, price
      - Methods: evaluation, tracking, pedagogical
      - Material elements
      - Timestamps and status
  
  2. Changes
    - Add training fields to user_profiles table
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'trainings' AND policyname = 'Users can read trainings'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'trainings' AND policyname = 'Admins can manage trainings'
  ) THEN
